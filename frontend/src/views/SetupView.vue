  ...
  <div class="min-h-screen flex items-center justify-center bg-gray-950 px-4">
    <div class="w-full max-w-md">
      <div class="text-center mb-8">
        ...
        <h1 class="mt-4 text-2xl font-bold text-white">{{ t('auth.setup.title') }}</h1>
        <p class="mt-1 text-gray-400">{{ t('auth.setup.subtitle') }}</p>
      </div>
      ...
      <form @submit.prevent="handleSetup" class="bg-gray-900 rounded-xl border border-gray-800 p-5 sm:p-8 space-y-5">
        ...
        <div>
          ...
          />
          <p class="mt-1 text-xs text-gray-500">{{ t('auth.setup.emailHint') }}</p>
        </div>
